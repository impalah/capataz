...
import type { ActionDefinition, CatalogImportResult, Service } from '@/api/types'
...
// La validación de forma de `config` la hace api/ al guardar; mientras el formulario no
// tenga campos reales para el resto, solo se ofrece `portainer` para no dejar crear acciones
// permanentemente inejecutables o rotas.
const supportedActionTypes = ['portainer']
...
const actionForm = ref<Partial<ActionDefinition>>({
  service_id: '',
  key: '',
  label: '',
...
const newService = () => {
  editing.value = false
  form.value = { id: '', name: '', group_name: 'Plataforma', environment: 'homelab', description: '' }
...
const saveService = async () => {
  ...
  try {
    if (editing.value && form.value.id) {
      // Solo los campos que este formulario realmente edita: nunca `{ ...service }` completo,
      // que arrastraría container_selectors/health_config/etc. al PATCH (CR-087). expected_version
      // activa la protección de concurrencia optimista del backend (CR-091).
      const { name, group_name, environment, description, version } = form.value
      await api.updateService(form.value.id, {
        name,
        group_name,
        environment,
        description,
    ...
    } else await api.createService(form.value)
...
const newAction = (serviceId?: string) => {
  ...
  actionForm.value = {
    service_id: serviceId ?? services.items[0]?.id ?? '',
    key: '',
    label: '',
    action_type: 'portainer',
    risk_level: 'read',
      ...
      <div class="catalog-grid">
        <article class="panel">
          ...
          <div class="q-mt-md q-gutter-sm">
            <q-btn outline color="primary" no-caps :label="t('pages.catalog.dryRun')" @click="dryRun" /><q-btn
              color="primary"
              no-caps
              :label="t('pages.catalog.import')"
              @click="applyImport"
            />
          </div>
          <q-banner
            ...
            >{{
              importResult.valid
                ? t('pages.catalog.importValid', { created: importResult.created, updated: importResult.updated })
            ...
            ><template #header
              ...
              ><q-item-section side class="row q-gutter-xs"
                ><q-btn
                  ...
                  @click.stop="editService(service)" /><q-btn
                  flat
                  round
                  color="negative"
                  icon="delete"
                  :aria-label="t('pages.catalog.deleteServiceAria', { name: service.name })"
                  @click.stop="requestRemoveService(service)" /></q-item-section></template
              ...
              ><q-item v-for="action in services.actionsByService[service.id] ?? []" :key="action.key"
                ...
                ><q-item-section side class="row q-gutter-xs"
                  ><q-btn
                    ...
                    @click="requestRemoveAction(action)" /></q-item-section></q-item
              ...
              ><q-item
                ><q-item-section
                  ><q-btn
                    ...
                    @click="newAction(service.id)" /></q-item-section></q-item
          ...
          ><q-form ref="serviceFormRef"
            ><q-card-section class="q-gutter-md"
              ><q-input
                ...
                outlined
                type="textarea"
                :label="t('pages.catalog.descriptionLabel')" /></q-card-section
            ><q-card-actions align="right"
              ><q-btn v-close-popup flat :label="t('common.cancel')" /><q-btn
                color="primary"
                no-caps
                :label="t('common.save')"
                @click="saveService" /></q-card-actions></q-form></q-card></q-dialog
          ...
          ><q-form ref="actionFormRef"
            ><q-card-section class="q-gutter-md"
              ><q-select
                ...
                :rules="[required]" /><q-select
                v-model="actionForm.action_type"
                :options="supportedActionTypes.map((value) => ({ label: t(`enums.actionType.${value}`), value }))"
                emit-value
                map-options
                outlined
                ...
                :rules="[required]" /><q-select
                v-if="actionForm.action_type === 'portainer'"
                ...
                :options="['read', 'operate', 'critical'].map((value) => ({ label: t(`enums.riskLevel.${value}`), value }))"
                emit-value
                map-options
                outlined
                :label="t('pages.catalog.riskLabel')"
                :rules="[required]" /><q-toggle
                v-model="actionForm.requires_confirmation"
                :label="t('pages.catalog.requiresConfirmation')" /></q-card-section
            ><q-card-actions align="right"
              ><q-btn v-close-popup flat :label="t('common.cancel')" /><q-btn
                color="primary"
                no-caps
                :label="t('common.save')"
                @click="saveAction" /></q-card-actions></q-form></q-card></q-dialog
      ><ConfirmDialog
        ...
        @confirm="removeService" /><ConfirmDialog
