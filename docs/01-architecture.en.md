...
- **Integrations**: every outbound destination (health URLs, connector URLs, SSH hosts) is validated against SSRF, integration credentials are encrypted catalog resources, and Portainer only operates on containers resolved from the catalog.
...
V1 uses a persistent Celery worker and Redis as broker/result backend. The API contains no Ansible or SSH tooling. Services reach Portainer, Prometheus, Grafana, Loki, HTTP health checks, Ansible and SSH through typed catalog **connectors**, whose credentials are **resources** encrypted in PostgreSQL ([ADR 008](adr/008-connectors-and-resources.en.md)); the API builds a client per connector (`ConnectorClientFactory`) and the runner decrypts an action's credentials only right before executing it. Grafana/Loki are resolved as deep links; Prometheus sits behind a `MetricsProviderPort`, one per `prometheus` connector, invoked from `StatusService.refresh` alongside the Portainer/health checks it already does — each service's catalog-declared `metrics` (admin-authored PromQL, one query per metric) are folded into the same `refresh-status` response, with no separate endpoint, cache, or unnecessary credentials. There is no cached `GET .../status` endpoint — every status read runs the real Portainer/health/Prometheus checks — see [infra/prometheus/README.md](../infra/prometheus/README.md). The migration to ephemeral Docker jobs is specified in [future-ephemeral-runner.md](11-future-ephemeral-runner.en.md), and the durable decisions live in the [ADRs](adr/).
