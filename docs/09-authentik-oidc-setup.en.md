...
- An Authentik instance with admin access (`https://authentik.404labo.net` in the examples below).
...
| Redirect URIs/Origins | `https://capataz.404labo.net/auth/callback` (strict) | Fixed path served by `AuthCallbackPage.vue`; adjust the host to `frontend`'s real domain. |
...
- Name: `Capataz`.
- Slug: `capataz` — this slug forms the issuer: `https://authentik.404labo.net/application/o/capataz/`.
...
domain(s) you assign it to, e.g. `authentik.404labo.net`).
...
3. Reload `https://authentik.404labo.net/if/flow/default-authentication-flow/` (or your
...
- **`Invalid OIDC access token` (403) on `/api/v1/auth/me`, with `[SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate` in the `api` logs**: Authentik (or the proxy in front of it) uses a certificate signed by an **internal** homelab CA, which the browser already knows about but the `api` container doesn't — it's a minimal Python image with only the standard public CAs. Fix it by trusting your CA from Capataz, **never** by disabling TLS verification:
  ```bash
  make trust-ca CA_URL=http://pi-dns.404labo.net/ca.crt   # downloads your CA and generates certs/ca-bundle.pem
  ```
  Add to `.env`:
  ```bash
